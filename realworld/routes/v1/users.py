...
from validation.auth import RegisterRequest, RegisterResponse, LoginRequest, ErrorResponse
import bcrypt
import uuid
...
def create_user() :
    ...
    if existing_user:
        return jsonify(ErrorResponse(**{"error": "Username already exists"})), 400
    elif existing_user_email:
        return jsonify(ErrorResponse(**{"error": "Email already exists"})), 400
...
def login_user() :
    ...
    if not user:
        return jsonify(ErrorResponse(**{"error": "email or password is incorrect"})), 400
    ...
    if bcrypt.checkpw(data.password.encode('utf-8'), user.password.encode('utf-8')):
        ...
        return jsonify(ErrorResponse(**{"error": "Invalid password"})), 401
...
def get_current_user():
    ...
    if user:
        ...
        return jsonify(ErrorResponse(**{"error": "Not logged in"})), 401
